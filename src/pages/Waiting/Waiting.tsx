...
function buildGreetingName(participant: ParticipantDoc | null, fallbackEmail?: string | null) {
  const candidate = participant?.displayName?.trim() || [participant?.firstName, participant?.lastName].filter(Boolean).join(" ").trim() || fallbackEmail?.split("@")[0] || "there";
...
export default function Waiting() {
  const { user, participant, loading, participantLoading, programState, programStateLoading, isWaitlisted, waitlistLoading } = useAuth();
  ...
  const handleMessageAdmins = () => {
    ...
    const body = user?.email ? encodeURIComponent(`Hi Admin Team,\n\nThis is ${user.email}.`) : "";
  ...
  if (isWaitlisted) {
    ...
    statusBody = "Thank you for filling out the registration form! The program is currently at capacity. You’ve been placed on the waitlist and will be notified when a spot opens up.";
    ...
    if (!matches_final && !started) {
      ...
      statusBody = "Thank you for filling out the registration form! We’re still working on finalizing everyone’s matches. You’ll receive an update as soon as your match is ready.";
    } else if (!matches_final && started) {
      statusTitle = "We’re still finalizing matches";
      statusBody = "Thank you for filling out the registration form! We’re still working on finalizing everyone’s matches. You’ll receive an update as soon as your match is ready.";
    } else if (matches_final && !started) {
      statusTitle = "Your match has been set!";
      statusBody = "Your match has been finalized. You’ll get full access to the program once it officially starts. Keep an eye on your email for the start date and next steps.";
    } else if (matches_final && started) {
    ...
    statusBody = "We’re unable to load program details right now. Please try again or contact support.";
  }
        ...
        {isLoading ? (
          ...
          <>
            ...
            {!isWaitlisted && programState?.matches_final && !programState?.started && (
              <>
                {matchLoading ? (
                  <div className={styles.loadingMessage}>Loading match info…</div>
                ) : partner ? (
                  <section className={styles.matchInfoSection}>
                    <div className={styles.matchInfoCard}>
                      ...
                      <div className={styles.matchInfoContent}>
                        ...
                        <h3 className={styles.matchName}>{partner.displayName}</h3>
                        <p className={styles.matchPhone}>{partner.phone_number}</p>
                      </div>
                    </div>
                  </section>
                ) : (
                  <div className={styles.loadingMessage}>
                    Match info unavailable. Please contact admins at <a href={`mailto:${ADMIN_EMAIL}`}>{ADMIN_EMAIL}</a>.
                  </div>
                )}
              </>
            )}
            ...
            <section className={styles.messageWrapper}>
              <div className={styles.messageCard}>
                {statusTitle && <h2 className={styles.statusTitle}>{statusTitle}</h2>}
        ...
        <button type="button" className={styles.messageButton} onClick={handleMessageAdmins}>
          <span className={styles.messageIcon} aria-hidden="true">
            <svg viewBox="0 0 24 24" focusable="false" role="presentation">
              <path d="M4 5.5A2.5 2.5 0 0 1 6.5 3h11A2.5 2.5 0 0 1 20 5.5v7A2.5 2.5 0 0 1 17.5 15H9.41l-4.2 3.36A.75.75 0 0 1 4 17.75z" fill="currentColor" />
            </svg>
          </span>
