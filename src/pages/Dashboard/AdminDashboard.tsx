...
export default function AdminDashboard() {
 ...
 const activeWeekData = useMemo(() => {
   ...
   allMatches.forEach((match) => {
     ...
     if (weekData && weekData.calls.includes(match.id)) {
       console.log("We have a match", match.id)
       variant = "green"; // Completed: at least one participant logged
     } else {
       // Compare program day-of-week to today's day-of-week (both 0-6)
       console.log("there is no match for", match.id)
       console.log("here is the whole week though")
 ...
 return (
   <div className={layoutStyles.page}>
     ...
     {/* pop-up window if clicking on a green meeting */}
     {activeMatch && (
       <CallLogModal
         match={activeMatch}
